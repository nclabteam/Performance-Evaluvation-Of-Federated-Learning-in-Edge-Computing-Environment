...
class Server:
    ...
    def fit(self, num_rounds: int, timeout: Optional[float]) -> History:
        ...
        #Measure the time during 1 round from cloud and add to a list, len == round - 1
        process_time_list = []
        ...
        time_in_a_round_list = []
        round_list = []
        for index,value in enumerate(process_time_list):
            round_list.append(index+1)
            if index == 0:
                time_in_a_round_list.append(value)
            else:
                time_in_around = process_time_list[index] - process_time_list[index-1]
                time_in_a_round_list.append(time_in_around)
        results = tuple(zip(round_list,time_in_a_round_list))
        timeline = tuple(zip(round_list,process_time_list))
        
        log(INFO, "Processing time on each round: %s", results)
        log(INFO, "Timer on each round: %s", timeline)
        return history
